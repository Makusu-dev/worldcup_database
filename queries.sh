...
echo "$($PSQL "SELECT name FROM teams FULL JOIN games ON team_id=winner_id OR team_id=opponent_id  WHERE year=2014 AND round='Eighth-Final' GROUP BY name;")"

echo -e "\nList of unique winning team names in the whole data set:"
echo "$($PSQL "SELECT name FROM teams RIGHT JOIN games ON team_id=winner_id GROUP BY name ORDER BY name;")"
